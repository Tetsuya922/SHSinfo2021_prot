...
class VolcanoViewController: UIViewController , XMLParserDelegate , UITableViewDataSource ,UITableViewDelegate {
    var parser : XMLParser!
    
    //複数の記事を格納するために配列
    var items = [ItemVolcano]()
    //Itemクラスのプロパティ　title,url,descriptionの２つのプロパティをもつ
    var item:ItemVolcano?
    ...
    func startDownload(){
        ...
        self.items = []
        var warnUrl:String = ""
        warnUrl = "http://weather.livedoor.com/forecast/rss/volcano.xml"
    ...
    func outDescripion(){
        ...
        for n in 1...num-1 {
            ...
            //5日前の日時
            let y:Date = Date(timeIntervalSinceNow: -60*60*24*5)
    ...
    var page2Controller : VolcanoViewController?
    ...
    func createPage2ViewController() -> VolcanoViewController{
        let sb:UIStoryboard = UIStoryboard(name: "VolcanoViewController",bundle: nil)
        let sController:VolcanoViewController = sb.instantiateInitialViewController() as! VolcanoViewController
        return sController
    }
